...
var app = express();
var result={'body': []};
var url = 'mongodb://MeenakshiBorusu:qwerty@ds057476.mlab.com:57476/test75';
app.use(cors());
...
app.post('/register', function (req, res) {
    ...
    var insertDocument = function(db, data, callback) {
        db.collection('Meenakshi').insertOne( data, function(err, result) {
            if(err)           {
...
app.post('/get-data',function (req,res) {
    ...
    var findUser = function(db, callback) {
        var cursor =db.collection('Meenakshi').find();
...
app.post('/update',function (req,res) {
    ...
    var updateDocument = function(db, data, callback) {
        db.collection('Meenakshi').updateOne({"_id":ObjectId(id)},{$set:item}, function(err, result) {
            if(err)            {
...
app.post('/delete', function(req, res) {
    ...
    MongoClient.connect(url, function(err, db) {
        if(err)        {
            res.write("Registration Failed");
            res.end();        }
        db.collection('Meenakshi').deleteOne({"_id": ObjectId(id)}, function(err, result) {
